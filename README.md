# ShieldGuard Pitch Deck, i-EIE 2026

Pitch deck and narration for **ShieldGuard, Vishing Detection Using Machine Learning and Natural Language Processing**, submitted to the International EUREKA Innovation Exhibition 2026 (Chapter A, ID IEIE2026_148) by Mohamad Ikmal Hafizi, UniKL MIIT.
...
`Exhibition.6.minit.mp4` is about 607 MB, which is over the GitHub file size limit, so it is not stored in this repository (see `.gitignore`). The demo slide loads it two ways: from the local file when present, and from a release URL as a fallback. Get the full quality file directly from the innovator.

## Keyboard shortcuts

`Space` play or pause, `A` auto advance, `T` script on or off, `H` hide all tools, `R` reset, arrow keys change slides.
